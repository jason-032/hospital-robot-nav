...
**Current state:** 1F reaches **71 of 74 attempted rooms (95.9%)**, verified against ground truth on 2026-09-16 (`~/sim_trials/2026-09-15/p8_clearance_sweep_1.*`): all 71 successes truly within 0.5 m (median 0.23 m), localisation within 0.035 m of truth, travel ratio 1.00 over 2,199 m, never inside a mapped obstacle. Failures: T01-17, S1383, T01-36. 2F recorded 83%, judged on odometry and unverified. **All 1F figures before this (38%, and 73 of 74 on 15 September) are withdrawn**, because the robot could not physically turn under Bullet Featherstone. Everything lives on branch `feat/1f-mesh-collision`. Treat any `sweep_test.py` SUCCESS as belief until checked with `tools/sim_checks/gt_analyse.py`.
...
`spatial_maps_1f.launch.py` runs the full 1F simulation. It loads `config/nav2_params_1f.yaml` after `nav2_params.yaml`, which is where every 1F-specific setting lives (polygon footprint, Smac State Lattice planner, 0.05 m global costmap on `/map_global`, inflation, doorway SpeedFilter, AMCL with `tf_broadcast: true`). Other floors are untouched by those. It is the launch file `sweep_test.py` runs have used; `headless:=true` runs Gazebo server-only.
...
**`AMCL tf_broadcast: false` on every floor except 1F,** where the 1F overlay sets it true and the static `map -> odom` publisher has been removed. Two things had to be right first: the 1F launch was republishing the latched map at the 1 Hz default, so AMCL reinitialised every second and left its transform stale (now 0.001 Hz), and AMCL's default motion noise let it diverge 13 m in the eastern hall (now alpha 0.02, 180 beams, 12 m range, 1000 to 5000 particles). It then held 0.035 m of ground truth across a full sweep. Do not enable it on another floor without checking both.
...
**2. Two limitations remain around pose and speed.** The faults that made 1F sweeps meaningless are fixed and the floor now verifies at 95.9% against ground truth, but:
(a) The doorway **SpeedFilter never changes the robot's speed.** It loads, receives filter info and mask, and the path crosses the door discs (573 in-zone samples), yet true speed is 0.45 m/s inside and outside zones alike. Config is in the 1F overlay and `maps/1F_speed_mask.*`; not yet diagnosed.
(b) **2F and the other floors are unverified.** They run DART, which moves the robot correctly in motion tests, but no floor except 1F has been checked with `gt_logger.py`. The 83% for 2F is odometry-judged.
Fixed on 1F, for the record, each of which had been hidden while the robot could not turn: `base_footprint` 0.15 m ahead of the wheel axle (up to 0.30 m of believed-pose shift per turn); `robot_radius: 0.27` against a 0.61 x 0.50 m body; the 1 Hz map republish; goals projected to 0.65 m clearance, which the honest footprint cannot occupy and then plan away from (83 of 94 goals failed "Start occupied"), now 0.85 m in `sweep_test.py` and `poi_nav_node.py`; and a behaviour tree whose `WouldA*RecoveryHelp` gate refused to recover from that state at all, removed in `navigate_to_pose_bt_1f.xml`.
...
Next phases, in order: diagnose the doorway SpeedFilter, check 2F against ground truth and migrate the other floors to the 1F configuration, implement multi-floor transition via elevators, add dynamic costmap layers from the LiDAR, extend to multi-robot, validate on a second IFC building.
