...
This implementation goes substantially beyond the original brief. Rather than validating a single floor, the system spans all 15 floors of the hospital (B1F through 14F), provides fully parameterised launch infrastructure, and includes an automated full-POI sweep test with cascade recovery. Floor 2F recorded an 83% sweep success rate (requires the upstream semantic.json dataset; see Installation). Sweep success is judged on the robot's odometry-based pose and has not yet been checked against Gazebo ground truth. Floor 1F recorded 38%, but ground-truth logging on 15 September 2026 showed that the 1F robot could not physically turn under the Bullet Featherstone physics engine, so every 1F sweep figure to date describes where the robot believed it was, not where it went (see Known Limitations). The physics cause has been fixed and 1F is being remeasured. A Dijkstra-based topological navigation module over the BIM-derived graph (5,542 nodes, 3,259 edges) is implemented as a standalone research prototype and is a planned integration target for the metric Nav2 stack. Each floor is independently launchable using a single parameterised entry point, and every floor carries its own occupancy map, semantic POI layer, topological network graph, and Gazebo simulation world.
...
**Empirical results**: Floor 2F recorded an 83% sweep success rate across the floor's full POI set. Floor 1F recorded 38%, and two sweeps on 15 September 2026 recorded 73 of 74 attempted goals. None of these figures is a physical navigation result. `sweep_test.py` judges success on the odometry-based pose, and a ground-truth logger run alongside a 1F sweep showed the robot never turned (true heading changed by at most 0.4°), drove 6.9 m straight from spawn into a wall and stayed there while Nav2 reported goals as reached. The 1F figures are therefore withdrawn. The 2F figure was measured under DART, which moved the robot correctly in motion tests, but it has not been checked against ground truth either. All measurements were obtained using `sweep_test.py` with the upstream `semantic.json` dataset (see External data dependency under Installation). The sweep test infrastructure and cascade recovery logic are fully implemented and reproducible given that dataset; the pass-rate figures depend on it and cannot be reproduced from repository contents alone.
...
With the fix in place, the first ground-truth-logged sweep exposed three further problems, which are open:
(a) The DiffDrive plugin's odometry follows the midpoint of the wheel axle, but `base_footprint` sits 0.15 m ahead of it. Every turn shifts the believed pose by up to 0.30 m (0.3·|sin(Δheading/2)|).
(b) The costmaps use `robot_radius: 0.27`, but the body reaches 0.36 m from `base_footprint`.
(c) AMCL TF broadcast is disabled, so nothing corrects (a).
Together these let the robot clip a mapped wall corner at 0.45 m/s, which spun it about 40° and, after further collisions, overturned it. The cascade recovery in `sweep_test.py` still mitigates start poses inside occupied cells, but sweep results should be read alongside `gt_analyse.py` output until these are resolved.
