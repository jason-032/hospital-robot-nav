...
This implementation goes substantially beyond the original brief. Rather than validating a single floor, the system spans all 15 floors of the hospital (B1F through 14F), provides fully parameterised launch infrastructure, and includes an automated full-POI sweep test with cascade recovery. Floor 1F reaches 71 of 74 attempted rooms (95.9%), verified against Gazebo ground truth: every success ended within 0.5 m of its goal in truth, and true distance travelled matched odometry over 2,199 m (requires the upstream semantic.json dataset; see Installation). Reaching that figure meant correcting faults that earlier sweep numbers had hidden, because success was judged on the robot's own odometry: under the Bullet Featherstone physics engine the 1F robot could not physically turn at all, so every 1F figure before 16 September 2026, including the 38% baseline, described where the robot believed it was rather than where it went, and those figures are withdrawn (see Known Limitations). Floor 2F recorded an 83% sweep success rate, also judged on odometry and not yet checked against ground truth. A Dijkstra-based topological navigation module over the BIM-derived graph (5,542 nodes, 3,259 edges) is implemented as a standalone research prototype and is a planned integration target for the metric Nav2 stack. Each floor is independently launchable using a single parameterised entry point, and every floor carries its own occupancy map, semantic POI layer, topological network graph, and Gazebo simulation world.
...
```
 ...
 |  1F: Bullet Featherstone physics, BIM wall mesh (collision), |
 |      invisible perimeter fence (collision-only, 4 box walls), |
 |      ground plane torsional friction 0 (Bullet needs this to  |
 |      let the robot turn at all)                               |
 +----------------------------+----------------------------------+
 ...
 |                           (1F: tf_broadcast: true, publishes  |
 |                            map->odom; other floors static TF) |
 |  planner_server       ->  NavfnPlanner / A* (0.5 m tol.)    |
 |                           (1F: Smac State Lattice, SE2        |
 |                            footprint, diff-drive primitives)  |
 |  controller_server    ->  RegulatedPurePursuit (0.45 m/s)    |
...
+-- spatial_maps/                   Multi-floor hospital navigation package
    ...
    +-- meshes/                          Floor 1F wall mesh (OBJ/GLB, visual + collision)
    +-- models/
    |   +-- pgm_walls_1f/                Gazebo model: 1108 PGM-derived collision
    |       +-- model.config             boxes for Floor 1F (generated by
    |       +-- model.sdf                pgm_to_sdf_walls.py, no longer included;
...
**Simulation environments**: 15 Gazebo Harmonic worlds (B1F through 14F) generated from BIM-derived occupancy maps. Each world is correctly formatted as SDF 1.10, uses the floor's occupancy grid as a ground plane texture, and launches via a single parameterised command. All 15 floors share identical world-frame coordinates, enabling consistent robot spawn and map alignment without per-floor calibration. Floor 1F uses the Bullet Featherstone physics engine, adopted when an earlier 17,051-face wall mesh carried collision geometry and crashed the default DART engine. DART also crashes on the current 7,122-face mesh, with both its ODE and Bullet collision detectors (tested 15 September 2026). The 1F ground plane sets torsional friction to 0, which Bullet requires for the robot to turn (see Robot model).

**Robot model**: A differential-drive mobile robot with a 2-DOF manipulator arm, RGB camera, and 360-degree GPU LiDAR, described entirely in URDF/Xacro with Gazebo plugin configuration for physics-accurate simulation at 1 kHz. Odometry is published at 50 Hz. The GPU LiDAR provides 360 samples at 10 Hz with a 12 m maximum range, and is bridged to ROS2 as a `sensor_msgs/LaserScan` on `/scan`. Wheel and caster contact surfaces are written for both physics engines, because Bullet Featherstone reads different SDF friction elements from DART. Bullet ignores `<ode><mu>`, sets friction once per link, and treats the torsional friction coefficient (SDF default 1.0) as spinning friction. With the defaults the robot could not turn at all and slipped 42% in a straight line. The caster is therefore kept as its own link with friction 0.1 for both engines, and torsional friction is 0 on the wheels and caster. In open-loop motion tests on 1F with these settings, Gazebo ground truth matches the commanded rotation and distance to within 0.2% under Bullet, as it already did under DART.
...
**Localization**: Floors other than 1F combine a static `map->odom` transform (published at spawn position 21.0, 38.0) with AMCL configured conservatively (`tf_broadcast: false`), so AMCL runs and subscribes to `/scan` without overriding the static transform. On 1F, AMCL now publishes `map->odom` and the static publisher is gone: across a full sweep its estimate stayed within 0.035 m of Gazebo ground truth on average and 0.18 m at worst. Two things had to be fixed first. The 1F launch republished the latched map at 1 Hz, which made AMCL reinitialise every second and leave its transform stale, and the default motion noise let AMCL diverge by 13 m in the eastern hall, where 77% of mapped obstacle cells have no physical geometry because the wall mesh contains only walls and columns. Odometry now matches ground truth to 0.1%, so 1F sets low motion noise, 180 beams and the LiDAR's true 12 m range. The `bond_timeout` for the Nav2 lifecycle manager was increased from the default 4 s to 30 s to prevent AMCL crash-loops caused by the 8.26 M cell (2075 x 3982 px) map blocking the executor for longer than the default heartbeat window.

**Floor 1F physical boundary**: An invisible perimeter fence (four collision-only box walls placed 1 m outside each PGM map edge) prevents the robot from physically escaping the hospital footprint during navigation, while having no effect on the Nav2 costmap, which is static-layer only. Interior physical walls come from the aligned BIM wall mesh (IfcWall and IfcColumn), which carries collision geometry; this change is under test. In a drive-into-wall test the mesh stopped the robot 0.316 m before the wall face, confirmed by Gazebo ground truth and LiDAR range. The earlier mesh could not carry collision because it included IfcSpace room volumes and closed door panels that trapped the robot. The regenerated mesh keeps all 33 carved doorways passable in offline analysis. Obstacles in the PGM that are not walls or columns (55 regions, 84.9 m²) remain in the costmap but have no collision geometry.

**PGM-to-SDF wall generator**: `pgm_to_sdf_walls.py` is a standalone utility that reads any PGM occupancy map and generates a Gazebo SDF model containing pixel-accurate collision wall boxes. At 3x downsampling (0.06 m/px), it produces 1,108 boxes for the 1F map in under one second. The generated model (`models/pgm_walls_1f/`) is no longer included by `1f.world`. In a drive-into-wall test the robot passed straight through its boxes (see Known Limitations), while the model held Gazebo's real-time factor at 0.13 against 1.00 with mesh collision (one idle measurement each).
...
**Nav2 autonomous navigation**: Full Nav2 stack (planner, controller, behaviour, smoother, BT navigator, lifecycle manager) operating on static BIM-derived occupancy maps with A* global planning and Regulated Pure Pursuit local control at 0.45 m/s (`desired_linear_vel` in `config/nav2_params.yaml`).
...
**Empirical results**: Floor 1F reaches 71 of 74 attempted rooms (95.9%), measured on 16 September 2026 and verified against Gazebo ground truth: all 71 successes ended within 0.5 m of the goal in truth (median 0.23 m, worst 0.35 m), true and believed pose agreed to 0.03 m at the goal, and true distance travelled matched odometry over 2,199 m (ratio 1.00). The robot spent none of the 11,051 logged samples inside a mapped obstacle. Sixteen rooms are skipped as stairwells, lifts and tagged zones. Floor 2F recorded an 83% sweep success rate, not yet checked against ground truth.

Earlier 1F figures are withdrawn. Floor 1F recorded 38%, and two sweeps on 15 September 2026 recorded 73 of 74 attempted goals, but none of those is a physical navigation result. `sweep_test.py` judges success on the odometry-based pose, and a ground-truth logger run alongside a 1F sweep showed the robot never turned (true heading changed by at most 0.4°), drove 6.9 m straight from spawn into a wall and stayed there while Nav2 reported goals as reached. Those 1F figures are withdrawn. The 2F figure was measured under DART, which moved the robot correctly in motion tests, but it has not been checked against ground truth either. All measurements were obtained using `sweep_test.py` with the upstream `semantic.json` dataset (see External data dependency under Installation). The sweep test infrastructure and cascade recovery logic are fully implemented and reproducible given that dataset; the pass-rate figures depend on it and cannot be reproduced from repository contents alone.
...
1. **Physical wall collision on the aligned mesh**: The 1F wall mesh now matches the PGM and carries collision in place of the 1,108 PGM-derived boxes. Remaining validation: repeat short trials and full sweeps in both configurations, confirm the robot passes every carved doorway, then extend mesh generation to the other floors (GLBs exist for B1F to 13F).

2. **Full AMCL localisation on the remaining floors**: 1F now runs AMCL with `tf_broadcast: true` and holds 0.035 m of ground truth across a sweep. The other 14 floors still rely on the static `map->odom` transform and should be migrated once their worlds are checked for the same map-versus-geometry mismatch that made AMCL diverge on 1F.
...
The original 1F wall mesh did not match the PGM. It had +0.9107 m added to X and contained IfcSpace room volumes and closed door panels, which together looked like an offset plus a scale error. The mesh has been regenerated from the pipeline GLB (walls and columns only, map frame). In simulation, the share of LiDAR returns within 0.06 m of occupied PGM cells rose from 53.2% to 99.3%, and returns more than 0.30 m from any occupied cell fell from 30.0% to 0.0%. The mesh now also carries collision (under test). Still pending: (a) repeat trials and full sweeps to validate the collision change; (b) AMCL TF broadcast is still disabled; (c) the 1F pass rate has not been remeasured. Only Floor 1F has a wall mesh, and the pipeline output contains no GLB for 14F.

**2. Sweep success is judged on believed pose, and 1F figures to date are not physical**
`sweep_test.py` records SUCCESS when Nav2 reports a goal reached, which depends on the odometry-based pose rather than the robot's true position. On 15 September 2026 a ground-truth logger (`tools/sim_checks/gt_logger.py`) showed that on 1F the robot could not physically turn under Bullet Featherstone: in open-loop tests its wheels turned at exactly the commanded rate while the body rotated 0.000 of the commanded angle and moved 0.58 of the commanded distance, whereas the same robot under DART matched both to 0.1%. Controlled experiments on a bare floor traced this to two contact settings (caster friction overwritten per link, and torsional friction treated as spinning friction), both now fixed (see Robot model). Every 1F sweep since the switch to Bullet on 28 May 2026 is affected, including the 38% baseline and the curve-wedging analysis of July 2026.

That fix exposed four further faults, all since corrected and all of which had been masked while the robot could not move physically. `base_footprint` sat 0.15 m ahead of the wheel axle that the DiffDrive plugin's odometry follows, so every turn shifted the believed pose by up to 0.30 m; the costmaps described a 0.27 m circle rather than the real 0.61 x 0.50 m body, so the robot clipped a mapped wall corner at 0.45 m/s and overturned; the 1F launch republished the latched map at 1 Hz, which kept AMCL reinitialising; and goals were projected to 0.65 m of clearance, closer than the honest footprint can occupy and still plan away from, which failed 83 of 94 goals in one sweep with "Start occupied". Sweep results should still be read alongside `gt_analyse.py` output, which is what established the 95.9% figure.

Two related limitations remain open. The doorway SpeedFilter loads its mask and receives filter info, but has never been observed changing the robot's speed: true speed is 0.45 m/s both inside and outside door zones across 573 in-zone samples. And 2F, along with the other floors, has not been checked against ground truth.

**2a. PGM-derived collision boxes do not stop the robot**
In a drive-into-wall test with `models/pgm_walls_1f/` loaded and no mesh collision, the robot passed through box sheets at x = 24.92 and x = 25.16 and continued to x = 33.19 without stalling. The cause is not yet confirmed; one hypothesis is that Bullet Featherstone does not build colliders for a static model made of 1,108 unjointed links, whereas the single-link wall mesh does collide. Results from configurations that relied on these boxes for physical walls should be read with this in mind.
...
The 83% (2F) and 38% (1F) navigation success rates were measured using `sweep_test.py` against the upstream `semantic.json` POI dataset, which is not included in this repository. The sweep test code and cascade recovery logic are fully implemented and reproducible; the figures themselves are dataset-dependent. The 1F figure is withdrawn and the 2F figure is unverified against ground truth (see item 2).
